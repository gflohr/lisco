...
int
chi_parse_move (chi_pos *pos, chi_move *move, const char *movestr)
{
	int match = 0;
	...
	*move = 0;
	...
	/* Only parse_san_move *may* return a valid error number.  */
	if (match < 0)
		return CHI_ERR_PARSER;
	else if (match > 0)
		return match;

	/* FIXME! This is not needed for SAN moves because we always take the
	 * move as generated by chi_legal_moves.
	 */
	fill_move(pos, move);
...
static int
parse_fq_move (chi_pos *pos, chi_move *move, const char *movestr)
{
...
static void
fill_move (chi_pos *pos, chi_move *move)
{
